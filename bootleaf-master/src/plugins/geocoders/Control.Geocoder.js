...
module.exports = {
	"class": L.Control.extend({
		...
		_geocodeResult: function (results, suggest) {
			if (!suggest && results.length === 1) {
				...
				for (var i = 0; i < results.length; i++) {
					...
					if (result !== undefined){
						this._alts.appendChild(this._createAlt(result, i));
					}
				}
			} else {
		...
		_geocode: function(suggest) {
			...
			if (this.options.geocoder.suffix !== undefined && searchTerm.toLowerCase().indexOf(this.options.geocoder.suffix.toLowerCase()) < 0){
				searchTerm += ", " + this.options.geocoder.suffix;
			}
			this._lastGeocode = searchTerm;
			if (!suggest) {
				this._clearResults();
			}

			this.fire('start' + mode);
			this.options.geocoder[mode](searchTerm, function(results) {
		...
		_keydown: function(e) {
			...
			default:
				...
				if (this.options.geocoder.suffix !== undefined && v.toLowerCase().indexOf(this.options.geocoder.suffix.toLowerCase()) < 0){
					v += ", " + this.options.geocoder.suffix;
				}
